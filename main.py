...
class CricketDarts:
    ...
    def get_num_players(self):
        ...
        self.input_buffer = ""
        while input_active:
            screen.fill(CHALKBOARD_BLACK)
            self.draw_text("Select number of players", WIDTH // 2, HEIGHT // 4, WHITE)
            self.draw_text(f"Players: {selected_players}", WIDTH // 2, HEIGHT // 2, WHITE)       
            self.draw_text("Press UP or DOWN to change, Enter to select", WIDTH // 2, HEIGHT * 3 // 4, WHITE)     
    ...
    def display_background(self):
        ...
        self.display_cricket_scoring(self.scores)
        pg.draw.rect(screen, WHITE, pg.Rect(330, 16, 368, 516), 2, 3) # Outer box
        pg.draw.rect(screen, WHITE, pg.Rect(330, 70, 368, 3)) # Horizontal separator line
        pg.draw.rect(screen, WHITE, pg.Rect(403, 16, 3, 516)) # Left player separator vertical line
        pg.draw.rect(screen, WHITE, pg.Rect(476, 16, 3, 516)) # Left vertical line
        pg.draw.rect(screen, WHITE, pg.Rect(549, 16, 3, 516)) # Right vertical line
        pg.draw.rect(screen, WHITE, pg.Rect(622, 16, 3, 516)) # Right player separator vertical line
        
        px_x = 486 #865
        px_y = 83
        px_y_multi = 66

        screen.blit(SCORE_FONT.render("20", True, WHITE), (px_x, px_y))
        screen.blit(SCORE_FONT.render("19", True, WHITE), (px_x, px_y + (px_y_multi * 1)))
        screen.blit(SCORE_FONT.render("18", True, WHITE), (px_x, px_y + (px_y_multi * 2)))
        screen.blit(SCORE_FONT.render("17", True, WHITE), (px_x, px_y + (px_y_multi * 3)))
        screen.blit(SCORE_FONT.render("16", True, WHITE), (px_x, px_y + (px_y_multi * 4)))
        screen.blit(SCORE_FONT.render("15", True, WHITE), (px_x, px_y + (px_y_multi * 5))) # 66px diff
        
        # Bullseye
        pg.draw.circle(screen, RED,(512, 495), 19)
        pg.draw.circle(screen, WHITE,(512, 495), 15)
        pg.draw.circle(screen, RED,(512, 495), 11)
        pg.draw.circle(screen, WHITE,(512, 495), 7)
        pg.draw.circle(screen, RED,(512, 495), 3)
    ...
    def render_scores(self, _player, _score_item, _score)    :
        ...
        if _player == 0:
            # Player 1
            score_px_x = 435
            screen.blit(SCORE_FONT.render("P1", True, WHITE), (score_px_x - 20, base_px_y - 22))
            score_px_y = base_px_y + (_score_item * y_px_diff)
            score_pixel = (score_px_x, score_px_y)
        elif _player == 1:
            # Player 2
            score_px_x = 580
            screen.blit(SCORE_FONT.render("P2", True, WHITE), (score_px_x - 20, base_px_y - 22))
            score_px_y = base_px_y + (_score_item * y_px_diff)
            score_pixel = (score_px_x, score_px_y)
        elif _player == 2:
            # Player 3
            score_px_x = 360
            screen.blit(SCORE_FONT.render("P3", True, WHITE), (score_px_x - 20, base_px_y - 22))
            score_px_y = base_px_y + (_score_item * y_px_diff)
            score_pixel = (score_px_x, score_px_y)
        elif _player == 3:
            # Player 4
            score_px_x = 655
            screen.blit(SCORE_FONT.render("P4", True, WHITE), (score_px_x - 20, base_px_y - 22))
            score_px_y = base_px_y + (_score_item * y_px_diff)
            score_pixel = (score_px_x, score_px_y)
        ...
        if _score == 1:
            ...
            pg.draw.line(screen, WHITE, start_pos, end_pos, line_width) # / First Point
        if _score == 2:
            start_pos = (score_pixel[0] - 10, score_pixel[1] + 10) # / First Point
            end_pos = (score_pixel[0] + 24, score_pixel[1] - 23)     # /
            pg.draw.line(screen, WHITE, start_pos, end_pos, line_width) # / First Point
            ...
            pg.draw.line(screen, WHITE, start_pos2, end_pos2, line_width) # \ Second Point
        if _score >= 3:
            start_pos = (score_pixel[0] - 10, score_pixel[1] + 10) # / First Point
            end_pos = (score_pixel[0] + 24, score_pixel[1] - 23)     # /
            start_pos2 = (score_pixel[0] - 10, score_pixel[1] - 23)   # \ Second Point
            end_pos2 = (score_pixel[0] + 24, score_pixel[1] + 10)     # \
            pg.draw.circle(screen, ORANGE,(score_pixel[0] +7, score_pixel[1] -6), circle_width) # DRAW CIRCLE
            pg.draw.circle(screen, CHALKBOARD_BLACK,(score_pixel[0] +7, score_pixel[1] -6), circle_width - line_width) # DRAW CIRCLE
            pg.draw.line(screen, ORANGE, start_pos, end_pos, line_width)
            pg.draw.line(screen, ORANGE, start_pos2, end_pos2, line_width)
    ...
    def display_game_state(self, _throw_count=0):
        ...
        for i, player in enumerate(self.players):
            if i == 0:
                text_width = PSCORE_FONT.render(str(self.total_scores[i]), True, WHITE)
                text_width = text_width.get_width()
                score = self.total_scores[i]
                #print(f"Score is: {score}")
                pgui.player_1_score(self, screen, FONT, PSCORE_FONT, text_width, score)
            if i == 1:
                text_width = PSCORE_FONT.render(str(self.total_scores[i]), True, WHITE)
                text_width = text_width.get_width()
                score = self.total_scores[i]
                pgui.player_2_score(self, screen, FONT, PSCORE_FONT, text_width, score)
            if i == 2:
                text_width = PSCORE_FONT.render(str(self.total_scores[i]), True, WHITE)
                text_width = text_width.get_width()
                score = self.total_scores[i]
                pgui.player_3_score(self, screen, FONT, PSCORE_FONT, text_width, score)
            if i == 3:
                text_width = PSCORE_FONT.render(str(self.total_scores[i]), True, WHITE)
                text_width = text_width.get_width()
                score = self.total_scores[i]
                pgui.player_4_score(self, screen, FONT, PSCORE_FONT, text_width, score)
        
        # Darts Used
        pgui.darts_used(self, screen, FONT, SCORE_FONT)
        ...
        if _throw_count >= 0 and _throw_count < 3:
            pg.draw.rect(screen, GREEN, pg.Rect(332, 546, 364, 32))
            pg.draw.rect(screen, WHITE, pg.Rect(330, 544, 368, 36), 2, 3)
            current_round_text = FONT.render(f"Round {CricketDarts.GAME_ROUND} - Player {self.current_player + 1} is up.", True, BLACK)
            for i in range(1,4):
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2) - i, (HEIGHT - 50) - i))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2), (HEIGHT - 50) - i))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2) + i, (HEIGHT - 50) - i))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2) - i, (HEIGHT - 50)))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2) + i, (HEIGHT - 50)))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2) - i, (HEIGHT - 50) + i))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2), (HEIGHT - 50) + i))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2) + i, (HEIGHT - 50) + i))

            current_round_text = FONT.render(f"Round {CricketDarts.GAME_ROUND} - Player {self.current_player + 1} is up.", True, WHITE)
            screen.blit(current_round_text, (WIDTH//2 - current_round_text.get_width()//2, HEIGHT - 50))
        else:
            pg.draw.rect(screen, RED, pg.Rect(332, 546, 364, 32))
            pg.draw.rect(screen, WHITE, pg.Rect(330, 544, 368, 36), 2, 3)
            current_round_text = FONT.render(f"Round {CricketDarts.GAME_ROUND} - Player {self.current_player + 1} is done.", True, BLACK)
            for i in range(1,4):
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2) - i, (HEIGHT - 50) - i))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2), (HEIGHT - 50) - i))
                screen.blit(current_round_text, ((WIDTH//2 - current_round_text.get_width()//2) + i, (HEIGHT - 50) - i))
            ...
            current_round_text = FONT.render(f"Round {CricketDarts.GAME_ROUND} - Player {self.current_player + 1} is done.", True, WHITE)
            screen.blit(current_round_text, (WIDTH//2 - current_round_text.get_width()//2, HEIGHT - 50))
